...
export default function AssessmentScreen() {
  ...
  // v0.5.1: support sampleSize param for Quick Start (balanced mini-blueprint across domains).
  ...
  const session = useMemo(() => {
    ...
    return { ...fullSession, questions: selectBalancedSample(fullSession.questions, sampleSize) };
  ...
  // Stop any in-flight speech when the user navigates away or moves to next question.
  useEffect(() => {
    return () => { stopSpeech(); };
  }, []);
  ...
  return (
    <Screen>
      ...
      <Text style={styles.privacyNote}>
        Generated session · seed {seed.slice(-8)} · Retake uses a new seed for equivalent but different variants.
      </Text>
    </Screen>
  );
}
...
const styles = StyleSheet.create({
  ...
  speakButton: {
    ...
    borderRadius: 999, borderWidth: 1, borderColor: '#C7D2FE'
  },
